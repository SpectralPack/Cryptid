				...
				local function update_dbl(area)
					...
					for i = 1, #area.cards do
						if
							...
							and type(area.cards[i].ability.immutable.other_side) == "table"
						then
							...
							if not G.P_CENTERS[area.cards[i].ability.immutable.other_side.key].immutable then
								if
									area.cards[i].ability.immutable.other_side
									and area.cards[i].edition.cry_double_sided
								then
									Cryptid.manipulate_table(
										...
										"other_side",
										{
											value = 1.5,
											type = "X",
											big = Cryptid.is_card_big({
												config = {
													center = G.P_CENTERS[area.cards[i].ability.immutable.other_side.key],
												},
											}),
										}
									)
					...
					elseif G.GAME.blind_on_deck == "Big" then
						...
						if G.GAME.current_round.cry_voucher_stickers.pinned == false then
							...
							if G.GAME.modifiers.cry_no_vouchers then
								very_fair_quip = pseudorandom_element(
									G.localization.misc.very_fair_quips,
									pseudoseed("cry_very_fair")
								)
							end
						end
