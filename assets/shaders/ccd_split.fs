...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	// Don't render outside rounded card corners
	if (pixel.a < 0.05) {
		return vec4(0.0);
	}

	// Diagonal split from bottom-left (0.0, 1.0) to top-right (1.0, 0.0):
	// Top-left half (uv.x + uv.y < 1.0) is transparent, revealing the Enhancement.
	if ((uv.x + uv.y) < 1.0) {
		return vec4(0.0);
	}

	// Bottom-right half (uv.x + uv.y >= 1.0): render the CCD consumable artwork
